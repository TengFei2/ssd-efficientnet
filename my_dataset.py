...
class YOLODataset(data.Dataset):
    ...
    '''
    def __init__(self, yolo_root, transforms=None, is_train=True): # 数据集文件夹camera2/images 包含训练集合和测试集
        super().__init__()
        if is_train:
            self.data_root = Path(yolo_root) / 'images/train'
            self.lables_root = Path(yolo_root) / 'labels/train'
            self.dataname = [i.stem for i in self.data_root.glob('*.jpg')]
        else:
            self.data_root = Path(yolo_root) / 'images/valid'
            self.lables_root = Path(yolo_root) / 'labels/valid'
            self.dataname = [i.stem for i in self.data_root.glob('*.jpg')]
    ...
    def __getitem__(self, index):
        image = Image.open(str(self.data_root / (self.dataname[index] + '.jpg')))
        ...
        # read txt
        boxes, labels = [], []
        with open(str(self.lables_root / (self.dataname[index] + '.txt')), 'r') as f:
